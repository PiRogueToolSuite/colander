...
export default {
  ...
  created() {
     this.$logger(this, 'PiRoguePackagesInfo');
     this.$debug('token', this.csrfToken, 'PiRogue', this.pirogueId);
     this.queryPiRogue();
  },
  methods: {
    queryPiRogue() {
      fetch(`/rest/pirogue/${this.pirogueId}/packages_info`, {
        method: 'GET',
        headers: {
    ...
    <div v-if="response.success">
      <table class="table table-striped table-sm w-100 table-hover">
        ...
        <tbody class="font-monospace">
          <tr v-for="item in response.content">
            <td class="w-75 pe-2">{{ item.package }}</td>
            <td class="pe-2">{{ item.version }}</td>
            <td class="pe-2">{{ item.state }}</td>
